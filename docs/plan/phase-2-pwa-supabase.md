...
> Đã thay thế hoàn toàn Phase 1 (Extension). Thư mục `extension/` đã bị xoá khỏi repo. Nội dung dưới đây mô tả đúng những gì đã build (xác nhận qua đọc code thật, không chỉ theo kế hoạch ban đầu) — các điểm khác biệt so với kế hoạch gốc được ghi rõ ở mục "Khác biệt so với kế hoạch ban đầu" cuối file.
...
## Schema dữ liệu thật (`supabase/schema.sql`)
...
## Tầng storage thật (`src/storage/supabaseStore.ts`)
