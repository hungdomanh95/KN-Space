    ...
    >
      ...
      {activeTab === 'general' && (
        <div className="settings-grid grid grid-cols-2 items-start gap-x-7 gap-y-[22px] max-md:grid-cols-1">
          ...
          <div className="setting-block mb-5">
            <label className="mb-2.5 flex items-center gap-[7px] text-[0.8438rem] font-bold uppercase tracking-[.03em] text-[var(--text-dim)]">
              <User className="icon h-[13px] w-[13px]" size={13} /> Tài khoản
            </label>
            <p className="hint mb-2.5 mt-0">{session?.user.email}</p>
            <button className="btn-ghost" onClick={() => showConfirm('Đăng xuất?', 'Bạn sẽ cần đăng nhập lại bằng email để vào dashboard.', signOut)}>
              <LogOut className="icon h-3.5 w-3.5" size={14} /> Đăng xuất
            </button>
          </div>
